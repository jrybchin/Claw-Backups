...
- The local Mission Control app still exists on `http://127.0.0.1:3001/`.
- The current live OpenClaw UI has been rolled back to a known-good bundle on `18889`; do not assume a native Mission Control route is present right now.
- The safe path for re-adding Mission Control UI is an isolated sidecar or a real source checkout, not direct compiled-bundle route surgery.
