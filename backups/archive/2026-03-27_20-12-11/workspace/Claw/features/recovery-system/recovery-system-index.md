...
### 5. OpenClaw Web UI Debugging Rules
**Purpose:** Incident-derived rules for debugging OpenClaw UI failures without repeating compiled-bundle crash loops.

**Sections:**
- **Purpose** — explains why this incident-specific debugging doc exists.
- **Confirmed Incident Pattern** — records the exact black-screen pattern, artifact trail, and known-good bundle markers.
- **What Actually Broke** — identifies the compiled-bundle workflow as the unstable step.
- **What Still Works** — separates the still-healthy surfaces from the broken path.
- **What Memory Got Right** — explains what the continuity system preserved successfully.
- **What Memory Missed** — explains where continuity over-reported success or missed the risk.
- **Debugging Rules** — gives the future-safe operating rules.
- **Instructions For Clawbot** — tells future agents how to approach OpenClaw UI work safely.
- **Memory Correction** — corrects stale assumptions about the native Mission Control route.
- **Next Safe Step** — defines the safest continuation path.

### 6. Codex ↔ Clawbot Runtime Truth Protocol
**Purpose:** Prevent Codex and Clawbot from silently diverging about the current live runtime truth.

**Sections:**
- **Purpose** — explains why runtime-truth reconciliation is required.
- **Core Rule** — states that silent runtime disagreement is unacceptable.
- **What Must Match Before Claiming Shared Understanding** — defines the minimum shared truth set.
- **Required Runtime Truth Checkpoints** — separates filesystem, served-runtime, and continuity truth.
- **Evidence Tagging Rule** — forces facts to be labeled by how they were confirmed.
- **Invalidated Assumptions Rule** — requires explicit recording of what is no longer safe to assume.
- **Reconciliation Trigger Rules** — defines when runtime-truth reconciliation must happen.
- **Required Read Order During Reconciliation** — gives the doc read order.
- **Operational Rule For Codex And Clawbot** — prevents category collapse between historical, desired, and current live state.
- **Mission Control / OpenClaw UI Specific Rule** — adds stricter requirements for UI truth claims.
- **Handoff Rule** — defines what must accompany runtime-sensitive handoffs.
- **Status** — marks the protocol as part of the continuity backbone.
