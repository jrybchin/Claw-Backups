...
During sweeps, this layer should be treated as a two-way consistency surface — but only after the local source-of-truth layer is coherent and only when Drive access/auth is actually available in the current runtime.
...
A key requirement now is strict runtime-truth reconciliation between Codex and Clawbot whenever runtime-sensitive work is happening. Shared docs are not enough if both agents are not looking at the same active truth.
...
### 7. Layered Memory Mirror Architecture
The memory system should use multiple parallel mirrors with clear role separation rather than forcing everything into one giant memory file.

Priority goals:
- stronger high-signal recall
- fast reconstruction after resets or handoffs
- lower noise and lower overlap between memory layers
- smoother coordination between local memory, current-state docs, Drive mirrors, backups, Mission Control, and multi-agent continuity
...
2. **Layered memory mirror architecture refinement**
3. **Memory backup / update system refinement and frequent-update file discipline**
4. **Document role classification**
5. **Memory and source-handling work**
6. **Codex shared-access / connection design plus finishing the GPT 5.3 backup bot path**
7. **Further response-logic refinement only after the above is cleaner**
