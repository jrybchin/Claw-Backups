...
- Mission Control now has an OpenClaw Control sidebar tab at `/mission-control`, implemented as a compiled control-ui patch that embeds the local dashboard from `http://127.0.0.1:3001/` with pop-out/new-tab fallback; the live served bundle is `~/.npm-global/lib/node_modules/openclaw/dist/control-ui/assets/index-Ij2djnNX.js` and a backup exists as `index-Ij2djnNX.js.bak-mission-control`.
