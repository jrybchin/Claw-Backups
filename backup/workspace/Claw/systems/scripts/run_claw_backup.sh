...
rm -rf backup
mkdir -p backup
cp -a "$SANITIZED"/. backup/
cp -a "$LOGFILE" backup/last-backup.log
cp -a "$MANIFEST" backup/manifest.txt

SUMMARY="$(git status --short backup | sed -n '1,10p' | tr '\n' '; ' || true)"
[ -n "$SUMMARY" ] || SUMMARY="refresh sanitized Claw backup"
...
git commit -m "Backup $DATE - $SUMMARY" >> "$LOGFILE" 2>&1
git push origin "$BRANCH" >> "$LOGFILE" 2>&1
send_telegram_notice "Claw backup OK ($DATE)"
